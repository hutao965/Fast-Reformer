...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.hf_model = hf_model_cpu.half().cuda()
        self.hf_model.eval()
        self.test_models = testmodels.TestModels_fp16(
            test_weight_dict, self.config.__dict__)
        self.test_kernels = testkernels.TestKernels_fp16(
            test_weight_dict, self.config.__dict__)
    ...
    def _assert_allclose(self, fast, hf, rtol=1e-5, atol=1e-3):
        np.testing.assert_allclose(
            fast.astype(np.half).reshape(-1),
            hf.astype(np.half).reshape(-1),
            rtol=rtol,
            atol=atol
        )
    ...
    def _random_hiddens(self, shape):
        return torch.randn(shape, dtype=torch.half)
    ...
    def test_bias_relu(self):
        ...
        hf_result = torch.nn.functional.relu(hiddens.cuda().half() + bias.cuda().half()).detach().cpu().numpy()
    ...
    def test_softmax(self):
        ...
        # tensor([-1e9])
        query_key_dots_gpu = query_key_dots.cuda().half()
        hf_result = torch.logsumexp(query_key_dots_gpu, dim=-1, keepdim=True)
        hf_result = torch.exp(query_key_dots_gpu - hf_result)
        fast_result = self.test_kernels.test_softmax(query_key_dots)
        self._assert_allclose(fast_result, hf_result.detach().cpu().numpy())
